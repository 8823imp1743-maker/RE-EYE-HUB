...
// pubDate 年齢フィルター：厳密に 48 時間 = 172800000 ms 超の item は捨てる
const MAX_ITEM_AGE_MS = 48 * 60 * 60 * 1000; // 48h（86400s × 2）
 ...
 * @param {{ fastJitter?: boolean, maxItemAgeMs?: number }} [opts] fastJitter / maxItemAgeMs（campaign 用に 14 日など）
 * @returns {Promise<IntelItem[]>}
 */
export async function scanKeyword(keyword, bypassDedup = false, opts = {}) {
  const url = GOOGLE_NEWS_RSS.replace('{KEYWORD}', encodeURIComponent(keyword));

  // 動的ジッター：通常 1〜3 秒 / fast 0.15〜0.5 秒
  const jitterMs = opts.fastJitter
  ...
  const maxItemAgeMs = opts.maxItemAgeMs != null ? opts.maxItemAgeMs : MAX_ITEM_AGE_MS;

  for (const raw of rawItems) {
    // ── pubDate 年齢フィルター（maxItemAgeMs 超は除外。campaign は scout から 14 日などを指定可） ──
    if (raw.pubDate) {
      const age = Date.now() - new Date(raw.pubDate).getTime();
      if (age > maxItemAgeMs) continue;
    }
...
/**
 * キーワードを順に巡回し、deadlineTs に達したら打ち切る（部分結果で 504 を避ける）。
 * @param {string[]} keywords
 * @param {boolean} bypassDedup
 * @param {number} deadlineTs  Date.now() + 残りミリ秒
 * @param {boolean} [fastJitter]
 * @param {{ maxItemAgeMs?: number }} [scanOpts] pubDate 窓（未指定時は MAX_ITEM_AGE_MS）
 * @returns {Promise<{ items: IntelItem[], errors: string[], totalFoundInFeed: number, truncated: boolean }>}
 */
export async function scanAllSequentialUntil(keywords, bypassDedup, deadlineTs, fastJitter = false, scanOpts = {}) {
  const items = [];
  const errors = [];
  let totalFoundInFeed = 0;
  const list = Array.isArray(keywords) ? keywords : [];
  let truncated = false;
  const kwOpts = { fastJitter, maxItemAgeMs: scanOpts.maxItemAgeMs };
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      const r = await scanKeyword(list[i], bypassDedup, kwOpts);
      items.push(...r.newItems);
      totalFoundInFeed += r.totalFromFeed;
    } catch (e) {
      const msg = `[${list[i]}] ${e?.message || 'Unknown error'}`;
      errors.push(msg);
      console.error('[rss-scanner]', msg);
    }
  }

  return { items, errors, totalFoundInFeed, truncated };
}
